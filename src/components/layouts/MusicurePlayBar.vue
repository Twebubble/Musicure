<template>
    <div class="play-bar" :class="{ show: !toggle }">
        <div class="fold" :class="{ turn: toggle }">
            <musicure-icon :icon="iconList.ZHEDIE" @click="toggle = !toggle"></musicure-icon>
        </div>
        <!--播放进度-->
        ...
        <div class="control-box">
            ...
            <div class="song-ctr">
                <musicure-icon class="musicure-play-show" :icon="playStateList[playStateIndex]"
                    @click="changePlayState"></musicure-icon>
                <!--上一首-->
                <musicure-icon class="musicure-play-show" :icon="iconList.SHANGYISHOU" @click="prev"></musicure-icon>
                <!--播放-->
                <musicure-icon :icon="playBtnIcon" @click="togglePlay"></musicure-icon>
                <!--下一首-->
                <musicure-icon class="musicure-play-show" :icon="iconList.XIAYISHOU" @click="next"></musicure-icon>
                <!--音量-->
                <el-dropdown class="musicure-play-show" trigger="click">
                    <musicure-icon v-if="volume !== 0" :icon="iconList.YINLIANG"></musicure-icon>
                    <musicure-icon v-else :icon="iconList.JINGYIN"></musicure-icon>
                    <template #dropdown>
                        <el-dropdown-menu>
                            <el-slider class="musicure-slider" style="height: 150px; margin: 10px 0" v-model="volume"
            ...
            <div class="song-ctr song-edit">
                <!--收藏-->
                <musicure-icon class="musicure-play-show" :class="{ active: isCollection }"
                    :icon="isCollection ? iconList.like : iconList.dislike" @click="changeCollection"></musicure-icon>
                <!--下载-->
                <musicure-icon class="musicure-play-show" :icon="iconList.download" @click="
                    downloadMusic({
                        songUrl,
                    ...
                    "></musicure-icon>
                <!--歌曲列表-->
                <musicure-icon :icon="iconList.LIEBIAO" @click="changeAside"></musicure-icon>
            </div>
        </div>
    </div>
</template>
...
@import "@/assets/css/musicure-play-bar.scss";
</style>
